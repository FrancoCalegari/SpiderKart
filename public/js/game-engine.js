...
(function () {
  ...
  let initSettings = { resolution: "1", graphics: "high" };
  try {
    if (window._skSettings) {
      initSettings = window._skSettings;
    } else {
      initSettings = JSON.parse(localStorage.getItem('sk_settings') || '{"resolution":"1","graphics":"high"}');
    }
  } catch(e) {}

  let antialiasSetting = (initSettings.graphics === "high");
  
  const renderer = new THREE.WebGLRenderer({ antialias: antialiasSetting });
  renderer.setSize(W, H);
  
  function applyResolution(resMultiplier) {
    renderer.setPixelRatio(Math.min(window.devicePixelRatio, 2) * parseFloat(resMultiplier));
  }
  applyResolution(initSettings.resolution);
  
  renderer.shadowMap.enabled = (initSettings.graphics === "high");
  renderer.shadowMap.type = THREE.PCFSoftShadowMap;
  container.appendChild(renderer.domElement);

  window.addEventListener('sk_settings_changed', (e) => {
    const s = e.detail;
    applyResolution(s.resolution);
    renderer.shadowMap.enabled = (s.graphics === "high");
    // Notar que el antialias requiere recrear el renderer en WebGL, 
    // pero pixelRatio y sombras se pueden cambiar en caliente.
    // Limpiamos shadowmaps si se desactivan
  ...
  // --- MARCADORES EN EL HUD PARA JUGADORES REMOTOS ---
  const playerMarkersContainer = document.getElementById('hud-player-markers');
  const playerMarkers = {};

  function updatePlayerMarkers() {
    if (!playerMarkersContainer) return;
    
    // Limpiar marcadores de karts desconectados
    ...
    for (const id in remoteKarts) {
      ...
      let marker = playerMarkers[id];
      if (!marker) {
        marker = document.createElement('div');
        marker.style.position = 'absolute';
        marker.style.padding = '2px 6px';
        marker.style.background = 'rgba(0,0,0,0.6)';
        marker.style.border = `1px solid ${e.color || '#ff00ff'}`;
        marker.style.color = '#fff';
        marker.style.fontFamily = 'Share Tech Mono, monospace';
        ...
        marker.textContent = e.name || 'PILOTO';
      ...
      pos.y += 1.8; // Un poco por encima del kart
      pos.project(camera);

      // pos.z > 1 significa que está detrás de la cámara
      let isBehind = pos.z > 1;
      if (isBehind) {
         pos.x *= -1;
         pos.y *= -1;
      }

      let screenX = (pos.x * halfW) + halfW;
      let screenY = -(pos.y * halfH) + halfH;

      // Clamp a los bordes de la pantalla si está fuera o detrás
      const margin = 24;
      let isOffscreen = false;
      if (screenX < margin || screenX > window.innerWidth - margin || 
          screenY < margin || screenY > window.innerHeight - margin || isBehind) {
          isOffscreen = true;
          ...
          // Distancia al borde
          const boundX = halfW - margin;
          const boundY = halfH - margin;
          
          const scale = Math.min(
              Math.abs(nx) > 0.001 ? boundX / Math.abs(nx) : Infinity,
              Math.abs(ny) > 0.001 ? boundY / Math.abs(ny) : Infinity
          );
          
          screenX = halfW + nx * scale;
          screenY = halfH + ny * scale;
      }

      marker.style.left = `${screenX}px`;
      marker.style.top = `${screenY}px`;
      
      // Estilo dinámico dependiendo de si está a la vista o no
      if (isOffscreen) {
  ...
  /* ──────────────────────────────────────────
     Input & Cross-Platform Support
  ────────────────────────────────────────── */
  const keys = {};
  const touchInput  = { forward:false, backward:false, left:false, right:false, jump:false, power:false, missile:false };
  let   gamepadInput = { forward:false, backward:false, left:false, right:false, jump:false, power:false, missile:false };
  // splitInput: recibe inputs del padre (split.html) vía postMessage cuando splitMode=1
  const splitInput  = { forward:false, backward:false, left:false, right:false, jump:false, power:false, missile:false };
  let currentPlatform = 'pc';

  function detectPlatform() {
    const ua = navigator.userAgent.toLowerCase();
    const isMobile = /android|webos|iphone|ipad|ipod|blackberry|iemobile|opera mini/i.test(ua) || navigator.maxTouchPoints > 0;
    const isConsole = /playstation|xbox|nintendo/i.test(ua);
    
    if (isMobile && !isConsole) {
  ...
  // URL params — debe estar antes de los listeners de teclado
  const urlParamsEngine = new URLSearchParams(window.location.search);
  const forcedGamepad = urlParamsEngine.has('gamepad') ? parseInt(urlParamsEngine.get('gamepad'), 10) : null;
  const isKeyboardOnly = urlParamsEngine.get('keyboard') === '1';
  const isSplitMode    = urlParamsEngine.get('splitMode') === '1';

  // Teclado — en splitMode el padre intercepta todo el teclado y lo envia via postMessage
  const allowKeyboard = !isSplitMode && (isKeyboardOnly || forcedGamepad === null);
  ...
  function setupTouchBtn(id, key) {
      ...
      if (!btn) return;
      btn.addEventListener('touchstart', e => { e.preventDefault(); touchInput[key] = true; }, {passive: false});
      btn.addEventListener('touchend', e => { e.preventDefault(); touchInput[key] = false; }, {passive: false});
  }
  setupTouchBtn('tc-left', 'left');
  ...
  // Modo split: recibir inputs del frame padre vía postMessage
  if (isSplitMode) {
    window.addEventListener('message', e => {
      if (e.data?.type === 'sk-input') {
        Object.assign(splitInput, e.data.input);
      }
    });
  }
  function pollGamepads() {
      // En splitMode, el frame padre (split.html) maneja todos los gamepads y
      // los envía vía postMessage. No polleamos aquí para evitar doble input.
      if (isKeyboardOnly || isSplitMode) return;
      const gamepads = navigator.getGamepads ? navigator.getGamepads() : [];
      gamepadInput = { forward: false, backward: false, left: false, right: false, jump: false, power: false };
      
      let targetGp = null;
      if (forcedGamepad !== null && gamepads[forcedGamepad]) {
          targetGp = gamepads[forcedGamepad];
      } else if (forcedGamepad === null) {
          // Default to first connected gamepad
          for (let i = 0; i < gamepads.length; i++) {
              if (gamepads[i]) { targetGp = gamepads[i]; break; }
          }
      }
      
      if (targetGp) {
          // Eje Izquierdo (X)
          if (targetGp.axes[0] < -0.3) gamepadInput.left = true;
          if (targetGp.axes[0] > 0.3) gamepadInput.right = true;
          
          // Botones (Distribución típica Xbox)
          if (targetGp.buttons[0] && targetGp.buttons[0].pressed) gamepadInput.forward = true; // A
          if (targetGp.buttons[1] && targetGp.buttons[1].pressed) gamepadInput.backward = true; // B
          if (targetGp.buttons[2] && targetGp.buttons[2].pressed) gamepadInput.power = true; // X
          if (targetGp.buttons[7] && targetGp.buttons[7].pressed) gamepadInput.forward = true; // RT
          if (targetGp.buttons[6] && targetGp.buttons[6].pressed) gamepadInput.backward = true; // LT
          if (targetGp.buttons[4] && targetGp.buttons[4].pressed) gamepadInput.jump = true; // LB
          if (targetGp.buttons[5] && targetGp.buttons[5].pressed) gamepadInput.jump = true; // RB
      }
  }
  ...
  function drawMinimap() {
    ...
    // Punto del jugador (Triángulo)
    const { mx: px, my: pz } = worldToMinimap(state.posX, state.posZ);
    minimapCtx.shadowColor = 'rgba(0,212,255,0.8)';
    minimapCtx.shadowBlur = 6;
    minimapCtx.beginPath();
    const size = 5;
    minimapCtx.moveTo(px + Math.cos(state.angle) * size, pz + Math.sin(state.angle) * size);
    minimapCtx.lineTo(px + Math.cos(state.angle + 2.4) * size, pz + Math.sin(state.angle + 2.4) * size);
    minimapCtx.lineTo(px + Math.cos(state.angle - 2.4) * size, pz + Math.sin(state.angle - 2.4) * size);
    minimapCtx.closePath();
    ...
    // Dibujar otros jugadores (Triángulos)
    for (const id in remoteKarts) {
      const e = remoteKarts[id];
      const { mx, my } = worldToMinimap(e.pos.x, e.pos.z);
      
      minimapCtx.fillStyle = e.color || '#ff00ff';
      minimapCtx.beginPath();
      const s = 4.5;
      minimapCtx.moveTo(mx + Math.cos(e.angle) * s, my + Math.sin(e.angle) * s);
      minimapCtx.lineTo(mx + Math.cos(e.angle + 2.4) * s, my + Math.sin(e.angle + 2.4) * s);
      minimapCtx.lineTo(mx + Math.cos(e.angle - 2.4) * s, my + Math.sin(e.angle - 2.4) * s);
      minimapCtx.closePath();
      minimapCtx.fill();
    }
  ...
  function update(dt) {
    ...
    const forward    = splitInput.forward  || keys['KeyW'] || (!isSplitMode && keys['ArrowUp'])    || touchInput.forward  || gamepadInput.forward;
    const backward   = splitInput.backward || keys['KeyS'] || (!isSplitMode && keys['ArrowDown'])  || touchInput.backward || gamepadInput.backward;
    const left       = splitInput.left     || keys['KeyA'] || (!isSplitMode && keys['ArrowLeft'])  || touchInput.left     || gamepadInput.left;
  ...
  window.SpiderKart = {
    ...
    applyRemoteState,
    removeRemotePlayer,
    resetRace(doLocalCountdown = true) {
      state.lap = 0;
